...
class PipelineEngine:
    ...
    def _create_destination(self, destination_type: str, config: Any) -> BaseDestination:
        """
        ...
        for pd in self._pipeline.destinations:
            if pd.destination:
                dest = self._create_destination(
                    pd.destination.type, pd.destination
                )
                dest.initialize()
                self._destinations[pd.destination_id] = dest
        
        self._logger.info(
            f"Pipeline {self._pipeline.name} initialized with "
    ...
    def run(self) -> None:
        ...
        try:
            ...
            PipelineMetadataRepository.upsert(
                self._pipeline_id, "ERROR", str(e)
            )
            raise
        finally:
            self._is_running = False
